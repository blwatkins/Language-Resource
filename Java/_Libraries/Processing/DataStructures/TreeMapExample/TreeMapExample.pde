...
TreeMap <String, PVector> map;
String[] names = {"one", "two", "three", "four"};
int count;

void setup() {
  size(500, 500);
  map = new TreeMap<String, PVector>(); // instantiate TreeMap
  count = 0;
  ...
  for (int i = 0; i < names.length; i++) {
    ...
    map.put(names[i], p); // add key-value pair to TreeMap
  }
...
void mousePressed() {
  ...
  if (count < names.length) {
    ...
    fill(lerpColor(color(0), color(0, 255, 0), (float)count / (names.length - 1)));
